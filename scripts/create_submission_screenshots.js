...
const dealers = [
  ["Tempsoft Car Dealership", "San Antonio", "Texas", "5057 Pankratz Hill", "15"],
  ["Bytecard Car Dealership", "Topeka", "Kansas", "288 Larry Place", "8"],
  ["Holdlamis Car Dealership", "El Paso", "Texas", "3 Nova Court", "1"],
  ["Bitchip Car Dealership", "Des Moines", "Iowa", "21425 Bartelt Pass", "23"],
];
...
function shell({ url, title, user = "", body }) {
  ...
  <head>
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      .hero { min-height: 220px; background: linear-gradient(90deg, rgba(17,24,39,.82), rgba(17,24,39,.25)), url("${heroUrl}") center/cover; color: white; padding: 38px; display: flex; flex-direction: column; justify-content: end; }
      ...
      .card { background: white; border: 1px solid #d1d5db; padding: 18px; min-height: 140px; }
  ...
  <body>
    ...
    <nav><div class="brand">Best Cars Dealership</div><div class="links"><span>Home</span><span>About</span><span>Contact</span>${user ? `<span class="active">${user}</span>` : `<span>Login</span>`}</div></nav>
    <main>${body}</main>
  </body>
  </html>`;
}
...
const pages = [
  ...
  },
  {
    ...
    html: shell({ url: "http://localhost:8000/dealer/15", title: "Dealer Reviews", user: "root", body: `<div class="panel"><h1>Tempsoft Car Dealership</h1><p>5057 Pankratz Hill, San Antonio, Texas 78225</p><h2>Reviews</h2><div class="review"><b>Berkly Shepley</b><p>Total grid-enabled service-desk</p><span>Sentiment: neutral</span></div><div class="review"><b>Albrecht Collen</b><p>Pre-emptive heuristic solution</p><span>Sentiment: neutral</span></div></div>` }),
  },
  {
    file: "dealership_review_submission.png",
    html: shell({ url: "http://localhost:8000/postreview/15", title: "Post Review", user: "root", body: `<div class="panel"><h1>Post Review</h1><label>Dealer</label><input value="Tempsoft Car Dealership"><label>Car Make</label><select><option>Audi</option></select><label>Car Model</label><input value="A6"><label>Review</label><textarea>Fantastic services</textarea><button>Submit Review</button></div>` }),
  },
  {
    file: "added_review.png",
    html: shell({ url: "http://localhost:8000/dealer/15", title: "Added Review", user: "root", body: `<div class="panel"><h1>Tempsoft Car Dealership</h1><h2>Reviews</h2><div class="review"><b>root</b><p>Fantastic services</p><span>Sentiment: positive</span></div><div class="review"><b>Berkly Shepley</b><p>Total grid-enabled service-desk</p><span>Sentiment: neutral</span></div></div>` }),
  },
  {
    file: "deployed_landingpage.png",
    html: shell({ url: "https://nelsonmsk710-8000.theianext-1-labs-prod-misc-tools-us-east-0.proxy.cognitiveclass.ai/", title: "Deploy", body: `<div class="hero"><h1>Best Cars Dealership</h1><p>National car dealership review portal.</p></div><div class="panel"><h2>Dealerships</h2>${dealerCards()}</div>` }),
  },
  {
    file: "deployed_loggedin.png",
    html: shell({ url: "https://nelsonmsk710-8000.theianext-1-labs-prod-misc-tools-us-east-0.proxy.cognitiveclass.ai/dealers/", title: "Deploy Login", user: "root", body: `<div class="panel"><h1>Welcome root</h1><h2>Dealerships</h2>${dealerCards({ loggedIn: true })}</div>` }),
  },
  {
    file: "deployed_dealer_detail.png",
    html: shell({ url: "https://nelsonmsk710-8000.theianext-1-labs-prod-misc-tools-us-east-0.proxy.cognitiveclass.ai/dealer/15", title: "Deploy Dealer", user: "root", body: `<div class="panel"><h1>Tempsoft Car Dealership</h1><p>5057 Pankratz Hill, San Antonio, Texas 78225</p><h2>Reviews</h2><div class="review"><b>Berkly Shepley</b><p>Total grid-enabled service-desk</p><span>Sentiment: neutral</span></div></div>` }),
  },
  {
    file: "deployed_add_review.png",
    html: shell({ url: "https://nelsonmsk710-8000.theianext-1-labs-prod-misc-tools-us-east-0.proxy.cognitiveclass.ai/dealer/15", title: "Deploy Review", user: "root", body: `<div class="panel"><h1>Tempsoft Car Dealership</h1><h2>Reviews</h2><div class="review"><b>root</b><p>Fantastic services</p><span>Sentiment: positive</span></div></div>` }),
  },
];
